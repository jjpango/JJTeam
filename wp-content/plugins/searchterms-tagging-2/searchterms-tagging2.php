...
function pk_stt2_db_get_number_of_posts_wo_traffic(){
	...
	if ( false == $post_count ) {
		global $wpdb;
		$sql = "SELECT count(`ID`) FROM $wpdb->posts WHERE `post_status` = %s AND `post_type` = %s AND ID NOT IN ( 
			SELECT post_id FROM ".$wpdb->prefix."stt2_meta );";
		$post_count = $wpdb->get_var($wpdb->prepare( $sql, 'publish', 'post' ));
...
function pk_stt2_db_get_id_post_wo_traffic(){
	global $wpdb;
	$sql = "SELECT `ID` FROM $wpdb->posts WHERE `post_status` = %s AND `post_type` = %s AND ID NOT IN ( 
		SELECT post_id FROM ".$wpdb->prefix."stt2_meta WHERE `post_id` != 0 ) LIMIT 1;";
	$post_ID = $wpdb->get_var($wpdb->prepare( $sql, 'publish', 'post'));	
	return $post_ID;
}
/**
...
function pk_stt2_db_get_last_promoted_post_title( $id ){
	...
	if ( false == $post_title ) {
		...
		$post_title = $wpdb->get_var($wpdb->prepare( $sql, $id ));	
...
function pk_stt2_function_wp_head_hook() {
	...
	if( 1 == intval(get_option('pk_stt2_enabled')) ){
		...
		if( $delimiter ){
			...
			{
				if (function_exists('banned_words')) {
					if (!check_bad_words($term))
					{pk_stt2_db_save_searchterms( $term );}
					}
				else {pk_stt2_db_save_searchterms( $term );}
			}
		}
			...
			if( !$popular ){
				if ( 1 == $options['auto_link'] ){									   
				   $permalink = get_permalink( $post->ID );		
					...
					$permalink = get_bloginfo( 'url' ).'/'.user_trailingslashit(pk_stt2_function_sanitize_search_link($term->meta_value)).'/';
				}
			} else {
